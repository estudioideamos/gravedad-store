    ...
    return array(
        ...
        array(
            ...
            'items' => array(
                ...
                array('Atributos (Juego, Rareza, Idioma, etc.)', 'En la ficha del producto, sección "Atributos": ahí se elige el juego, la rareza, el idioma, el tipo de producto, etc. Esto es lo que hace que el producto aparezca al usar los filtros y los menús desplegables. (Para que estos atributos se agreguen solos según la categoría, mirá "Atributos automáticos" más abajo).'),
                ...
                array('Cartel de "NUEVO"', 'Aparece solo en los primeros 30 días desde que publicás el producto (Productos → Añadir producto → Publicar). Pasado ese mes se saca solo, sin que tengas que hacer nada. Si el producto está en oferta, se muestra el cartel de "OFERTA" en su lugar (nunca los dos juntos).'),
                array('¿Qué es el SKU?', 'El código interno que identifica a cada producto (no lo ve el cliente en la tienda). Sirve para buscarlo rápido, para que la carga masiva por CSV reconozca si tiene que actualizar un producto existente en vez de duplicarlo, y para ordenar el stock cuando hay variantes parecidas.'),
                array('Formato recomendado de SKU', 'JUEGO-PRODUCTO-NÚMERO, todo en mayúsculas separado por guiones. Ej: POK-CHARIZARDX-125. JUEGO es una abreviación de 3 letras (POK, MTG, YGO, OP, DB, DIG, JM para juegos de mesa), PRODUCTO es el nombre corto sin espacios ni acentos, y NÚMERO es el número de colección de la carta (o un correlativo propio para sellado/accesorios).'),
            ),
        ),
        array(
            'icon' => '📤',
            'title' => 'Carga masiva de productos (CSV)',
            'where' => 'Menú lateral → Productos → Importar / Exportar',
            'items' => array(
                array('¿Qué es?', 'Para cargar muchos productos de una sola vez sin hacerlo uno por uno: Productos → botón "Importar" (arriba, al lado de "Añadir producto"). Ahí subís un archivo CSV (una planilla de Excel/Google Sheets guardada como CSV) con una fila por producto.'),
        ...
        array(
            'icon' => '🏷️',
            'title' => 'Atributos automáticos',
            'where' => 'Menú lateral → Editar Sitio → Atributos automáticos',
            'items' => array(
                array('¿Qué es?', 'Ahí armás "reglas": cada una dice "si el producto está en esta categoría, agregale estos atributos" (vacíos, listos para elegir el valor), así no hay que sumarlos a mano en cada producto. Se completan al guardar el producto (Publicar o Guardar borrador).'),
                array('Cómo se usa, paso a paso', '1) Cargá el producto y marcale la categoría (ej: "Cartas sueltas"). 2) Dale Publicar o Guardar borrador — tiene que guardarse una vez. 3) Volvé a entrar al producto (o recargá la página) y andá a "Datos del producto" → pestaña "Atributos": ahí ya vas a ver agregados solos los atributos que correspondan a esa categoría (vacíos), listos para que despliegues cada uno y elijas el valor. Importante: no aparecen antes de guardar por primera vez, tienen que estar categoría + guardado para que se agreguen.'),
                array('Varias reglas a la vez', 'Con el botón "+ Agregar otra regla" podés sumar una regla por cada categoría que necesites (ej: una para "Cartas sueltas" con Colección/Idioma/Rareza/Condición/Juego, y otra distinta para "Juegos de mesa" con Editorial/Cantidad de jugadores). Si un producto está en varias categorías con regla propia, recibe la suma de los atributos de todas las que le apliquen.'),
                array('Quitar o desactivar una regla', 'El botón "✕" borra esa regla del todo. El tildado "Regla activa" la deja guardada pero sin efecto, por si querés pausarla y volver a usarla más adelante.'),
                array('Apagar todo de una', 'El interruptor "Activado" de arriba de todo desactiva todas las reglas a la vez, sin perder cómo las configuraste, por si en algún momento no querés que se agregue nada automáticamente.'),
            ),
        ),
        array(
            'icon' => '📸',
            'title' => 'Fotos de producto: formato y optimización',
            'where' => 'Al cargar la imagen destacada de un producto',
            'items' => array(
