...
$bloque_iconos = array('clock', 'shield');
?>
<main class="singles-page">
  ...
  <div class="content-shell info-shell">
    <div class="info-steps">
      <?php foreach ($env_def['zonas'] as $i => $zona): $n = $i + 1;
        $nombre = gravedad_content_panel_opt('envios', 'zona' . $n . '_nombre', $zona['nombre']);
        $tiempo = gravedad_content_panel_opt('envios', 'zona' . $n . '_tiempo', $zona['tiempo']);
        $costo = gravedad_content_panel_opt('envios', 'zona' . $n . '_costo', $zona['costo']);
        if (!$nombre) { continue; }
      ?>
      <div class="info-step"><span class="step-icon"><?php echo gravedad_icon($zona_iconos[$i]); ?></span><h3><?php echo esc_html($nombre); ?></h3><p><?php echo esc_html($tiempo); ?><br><b class="step-highlight"><?php echo esc_html($costo); ?></b></p></div>
      <?php endforeach; ?>
    </div>
    <p class="info-note">El costo final y el tiempo estimado se confirman al finalizar la compra, según tu código postal.</p>
    <?php foreach ($env_def['bloques'] as $i => $bloque): $n = $i + 1;
      $titulo = gravedad_content_panel_opt('envios', 'bloque' . $n . '_titulo', $bloque['titulo']);
      $texto = gravedad_content_panel_opt('envios', 'bloque' . $n . '_texto', $bloque['texto']);
      if (!$titulo) { continue; }
